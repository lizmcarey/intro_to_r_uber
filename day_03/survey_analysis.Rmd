...
```
What do you notice? We can see the min and max of the start dates for the survey, we can see that Q4 has 7 missing values, Q6 has 54 missing values, 41% of the responses came from ci = 1, and there are 12 of the same email in column `e.` 

This report is helpful in seeing if there are any anomalies or large missing values in the data set. Sometime this report will catch whether we've read the data incorrectly and stop us before we analyze a broken data set. 
...
```{r}
#store a function that calculates the margin of error where p = sample proportion, n = sample size, and z is the critical value (z-score). 
...
Q4_table
```
We notice that 5 counts are `NA` which means 5 respondents skipped this question. Let's be sure to remove these null values from our total n size. 
...
Q4_table %>% 
  ...
  scale_x_discrete(labels = function(x) str_wrap(x, width = 10))
```
Let's look at Q5's satisfaction question.  
```{r}
Q5_table <- df_raw %>% 
  group_by(Q5) %>% 
  summarize(n = n()) %>% 
