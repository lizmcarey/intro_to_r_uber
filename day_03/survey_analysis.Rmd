...
```
What do you notice? We can see the min and max of the start dates for the survey, we can see that Q4 has some missing values, Q6 has missing values, 41% of the responses came from ci = 1, and there are 12 of the same emails in column `e.` 
...
From the summary report, it looks like there are a few duplicate emails. Let's count how many duplicates there are. 
...
Let's double check that we've removed all of the duplicates
...
#lets store the join data sets as df_raw
df_raw <- 
  df_raw %>% 
  left_join(demo_data, by = c("e" = "email"))
...
Now, let's calculcate a frequency table of the responses from Question 1, with the margin of error for each response.
...
Q1_table %>% 
  ...
  geom_col()
```
Now let's get fancy. Let's color the bars, add the error bars, add labels to the bars and to the graph.

To understand what each line of code does, try commenting out a line and rerunning the code. Can you spot what's different? 
...
You'll need to transform the data from wide to long before grouping and calculating the frequencies. 
...
Q4_table
```
We notice that a few counts are `NA` which means a few respondents skipped this question. Let's be sure to remove these null values from our total n size. 
...
Q4_table %>% 
  ...
  scale_x_discrete(labels = function(x) str_wrap(x, width = 10))
```
Let's look at Q5's satisfaction question.  We'll want to relevel the response options so they are plotted in order. 
```{r}
Q5_table <- df_raw %>% 
  group_by(Q5) %>% 
  summarize(n = n()) %>% 
...
Q6_table
```
Looks like we have respondents who skipped this question. We want the proportion of those who *answered* the question. So let's remove those who skipped the question from our total n size. 
